$(document).ready(function() {
    ...
    // vide les input avec la class 'clearinput' au focus
    ...
    //        $.datepicker.setDefaults( $.datepicker.regional[ "fr" ] );
            $( "#c_date" ).datepicker({
                    showOn: "button",
                    buttonImage: "images/style/icon-calendar.png",
                    buttonImageOnly: true
            });
            $( "#c_date" ).datepicker( "option", "dateFormat", 'd MM, yy' );
        });
    }
...
function clearInput(classname){
    var champsmagicclic = new Array();
    $(classname).each(function(i){
        if($(this).attr('id') && $(this).attr('id')!=''){
            champsmagicclic[$(this).attr('id')] = $(this).attr('value');
            $(this).attr('value', '')
        }
    });
    for(var idch in champsmagicclic) {
        if($('#'+idch) && $('#'+idch).attr('value') == ''){
            $('#'+idch).attr('value', champsmagicclic[idch]);
            $('#'+idch).focus(function(){
                if($('#'+idch).attr('value') == champsmagicclic[$('#'+idch).attr('id')])
                    $('#'+idch).attr('value', '');
            });
            $('#'+idch).blur(function(){
                if($('#'+idch).attr('value') == '')
                    $('#'+idch).attr('value', champsmagicclic[$('#'+idch).attr('id')]);
            });
